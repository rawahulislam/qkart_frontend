...
let allAddresses = addresses.all



const AddressInput = allAddresses.map((e)=>(<Box key={e._id} className="address-item" borderColor={
  addresses.selected === e._id ? "#00A278" : "#ddd"
}
backgroundColor={
  addresses.selected === e._id ? "#E9F5E1" : "white"
}
mt={1}><Button type="text"
role="text"
variant="text"  sx={{ color: "black" }} onClick={()=>{setAddresses({...addresses, selected : e._id})}}>
  {e.address}</Button>
  <Box>
  <Button onClick={()=>{deleteAddress(token, e._id)}}><Delete/>DELETE</Button></Box></Box>))
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      let response = await axios.post(
        `${config.endpoint}/user/addresses`,{ address : newAddress},
  ...
  const performCheckout = async (token, items, addresses) => {
    if(validateRequest(items, addresses)){
      ...
      let money = localStorage.getItem("balance")
      localStorage.setItem("balance" , money-getTotalCartValue(items))
      enqueueSnackbar(
      ...
      return response.data
          ...
          enqueueSnackbar(
            "Please check the order details or backend has stopped",
            {
              variant: "error",
            }
        )
        ...
        <Grid item xs={12} md={9}>
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              ...
              {addresses.all.length > 0 && AddressInput}
